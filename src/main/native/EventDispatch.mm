...
#include "com_martijncourteaux_multitouchgestures_EventDispatch.h"
...
CGEventRef eventTapCallback(CGEventTapProxy proxy, CGEventType type, CGEventRef eventRef, void *refcon) {
    ...
    switch ([event type])
    {
        ...
        case NSEventTypeSwipe:
            printf("Swipe: X = %10.6f; Y = %10.6f", event.deltaX, event.deltaY);
            fflush(stdout);
            break;
...
printf("[start] eventTap: 0x%lx, %ld, %ld, %ld, %016lx, 0x%lx\n", (long) eventTap, kCGSessionEventTap, kCGHeadInsertEventTap, kCGEventTapOptionListenOnly, kCGEventMaskForAllEvents, kCFAllocatorDefault);
fflush(stdout);
...
printf("[start] runningLoop: 0x%lx, %s\n", (long) runningLoop, [(__bridge NSString *)kCFRunLoopCommonModes UTF8String]);
fflush(stdout);
        CFRunLoopRun();
        running = true;
    }
